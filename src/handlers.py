...
def file_notifier(message):
    ...
    if os.name == 'nt':
        try:
            ...
            toaster.show_toast(message['header'], message['content'])

        except ImportError:
            append_log('Error! win10toast module not found')

        except Exception as e:
            append_log('Error! In function file_edit_notifier ' +
                       str(e))

    # if you are using Linux
    elif os.name == 'posix':
        os.system('notify-send "{0}" "{1}"'
                  .format(message['header'], message['content']))
...
def create_new_folder(args, file_prop):
    ...
    try:
        ...
        validate_path = file_prop['path'].replace('\\', '/').replace('//', '/')

        if file_prop['is_dir'] and validate_path == args['path_to_folder']:

            import os

            os.mkdir(file_prop['path'])
